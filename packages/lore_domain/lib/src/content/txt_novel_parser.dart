...
abstract final class TxtNovelParser {
  ...
  /// 行首章节标题（trim 前允许少量前导空白；按优先级匹配其一即视为标题）：
  /// 1. `第N章/第N节/第N回`，N 可为阿拉伯或中文数字（一二三…百千）；前可带
  ///    可选的 `第M卷 [卷名] ` 前缀（合并标题，如 `第六卷 破茧成蝶 第一章 …`），
  ///    卷名不计入副标题。
  /// 2. `Chapter N`（大小写不敏感）；
  ...
  static final RegExp _headingPattern = RegExp(
    r'^[\t 　]*(?:'
    // 可选「第M卷 [卷名]」前缀，后跟必须的章节 token；[^\n第]*? 确保
    // 卷名里的内容不会吞掉章节的「第」，且不跨行。
    r'(?:第\s*[一二三四五六七八九十百千两零〇0-9]+\s*卷[^\n第]*?)?'
    r'第\s*[一二三四五六七八九十百千两零〇0-9]+\s*[章节回]'
    r'|chapter\s*[0-9]+'
