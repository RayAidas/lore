...
业务模型不再假设绝对文件路径，Android 与本地目录共享存储契约、冲突语义和迁移测试。macOS 与 Android 现在统一使用可移植语义仓储；macOS 通过本地目录存储会话和原生文件协调网关保留路径平台能力，不再维护独立的 path-specific 语义仓储。所有新增跨平台行为必须先落在通用存储契约上。

源码文件长度检查沿用仓库原有的 1000 行默认阈值，并保持为手动检查。可移植语义仓储已按书库、文档、垃圾箱和共享支撑职责拆分，不再需要文件长度白名单。
